...
let streak3 = 0, streak4 = 0, streak5plus = 0, streak2 = 0, rpAppearances = 0;
...
// --- 先発の登板間隔 ---
let gaps5orLess = 0, gap6 = 0, gap7to8 = 0, gap9 = 0, gap10plus = 0, gapTotal = 0;
...
const sd = Math.sqrt(pitchesPerStart.reduce((a, b) => a + (b - mean) ** 2, 0) / pitchesPerStart.length);
const level = leagueLevel(season);

console.log(`=== 投手起用と走塁の実測 (seed ${season.masterSeed}, ${teams}球団) ===`);
console.log(`救援の連投（連続登板の塊の数）: 2連投 ${streak2} / 3連投 ${streak3} / 4連投 ${streak4} / 5連投以上 ${streak5plus}`);
console.log(`  1チームあたり: 3連投 ${(streak3 / teams).toFixed(1)} / 4連投 ${(streak4 / teams).toFixed(1)} / 5連投以上 ${(streak5plus / teams).toFixed(1)}   (目標: 3連投 1〜2、4連投以上 0)`);
console.log(`  2連投の登板が救援登板に占める割合 ${pct(streak2 * 2, rpAppearances)}   (目標: 主力の登板の18〜19%)`);
console.log(`完投 ${completeGames} (1チーム ${(completeGames / teams).toFixed(2)})   (目標: 6〜8)`);
console.log(`先発の球数: 平均 ${mean.toFixed(1)} σ ${sd.toFixed(1)} 90%点 ${sorted[Math.floor(sorted.length * 0.9)].toFixed(0)} 最多 ${maxPitches.toFixed(0)} (${maxPitchesBy})   (目標: 平均93、最多137〜143、σ≈17)`);
console.log(`先発の登板間隔: 中5日以下 ${pct(gaps5orLess, gapTotal)} / 中6日 ${pct(gap6, gapTotal)} / 中7〜8日 ${pct(gap7to8, gapTotal)} / 中9日 ${pct(gap9, gapTotal)} / 中10日以上 ${pct(gap10plus, gapTotal)}`);
console.log(`  (目標: 3〜4% / 49〜53% / 16〜20% / — / 22〜23%)`);
console.log(`盗塁企図 ${level.stealAttemptsPerTeam.toFixed(1)}/チーム 成功率 ${level.stealSuccessRate.toFixed(3)}   (目標: 105〜110 / .65〜.75)`);
