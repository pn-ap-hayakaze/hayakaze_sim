...
const AVERAGE_PITCHER: Player = {
  ...
  pitcherRole: 'SP',
  ratings: {
    batting: { meetVsR: 50, meetVsL: 50, powerVsR: 50, powerVsL: 50, contact: 50, eye: 50, clutch: 50 },
    ...
    durability: 50,
    pitching: {
      hits: 50, homeRuns: 50, strikeouts: 50, walks: 50, stamina: 50, recovery: 50, clutch: 50,
      arsenal: [], speed: 50, breakAmount: 50, control: 50,
    },
  },
};
...
export function analyzeClutch(rosters: Roster[]): ClutchAnalysis {
  ...
  const platoonNormal = batters.map((b) => expectedWoba(b, 'L', false) - expectedWoba(b, 'R', false));
