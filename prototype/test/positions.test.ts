...
import { ALL_FRESH, simulateGame } from '../src/engine/sim/game.js';
...
import {
  ...
  type LineupSlot,
} from '../src/engine/player/ratings.js';
import { Rng } from '../src/engine/rng.js';
import { emptyBatting, addBatting } from '../src/engine/sim/stats.js';
...
describe('守備位置と打順枠の型', () => {
  ...
  it('FieldingByPosition に DH は存在しない（型レベル）', () => {
    const fielding: FieldingByPosition = {
      P: 1, C: 1, '1B': 1, '2B': 1, '3B': 1, SS: 1, LF: 1, CF: 1, RF: 1,
    };
...
describe('試合での出場枠の記録', () => {
  ...
  function slotsOf(result: ReturnType<typeof play>, teamId: string) {
    ...
    for (const [id, s] of result.batting) {
      const player = [...home.batters, ...home.pitchers, ...away.batters, ...away.pitchers].find((p) => p.id === id)!;
