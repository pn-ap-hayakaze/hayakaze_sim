...
import { createRngStreams, deriveSeed, Rng, RNG_PURPOSE, type RngStreams } from '../src/engine/rng.js';
import { advanceToEnd, createSeason } from '../src/engine/league/season.js';
import type { GameResult } from '../src/engine/sim/game.js';
...
describe('試合ごとの乱数の独立性', () => {
  ...
  it('翌日のうち摂動した試合の2球団が関わらない試合は同一', () => {
    ...
    const games = normal.schedule.filter(
      (g) => g.day === PERTURB_DAY + 1 && !involved.has(g.homeTeamId) && !involved.has(g.awayTeamId),
    );
