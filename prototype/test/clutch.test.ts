...
import { generateLeague } from '../src/engine/player/generate.js';
import { analyzeClutch } from '../src/engine/metrics/clutchAnalysis.js';
import { leagueLevel } from '../src/engine/metrics/leagueLevel.js';
import { clutchFactor } from '../src/engine/sim/profile.js';
import { CLUTCH_CEILING, drawClutch } from '../src/engine/player/clutch.js';
...
describe('真のクラッチ才能（解析値）', () => {
  ...
  it('スタメンの（得点圏 wOBA − 通常 wOBA）の選手間 σ が 6〜8 wOBA ポイント', () => {
    ...
    expect(sd, `シードごと: ${results.map((r) => r.talentSd.toFixed(1)).join(' / ')}`).toBeGreaterThanOrEqual(6);
...
describe('リーグ得点の保存', () => {
  it('係数あり と 全員クラッチ50 で 6シード平均の 1試合平均得点の差が ±1%', () => {
    ...
    expect(ratio, `係数あり ${(withClutch / SEEDS.length).toFixed(3)} / 全員50 ${(neutral / SEEDS.length).toFixed(3)}`).toBeGreaterThan(0.99);
