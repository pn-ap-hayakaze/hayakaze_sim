...
import { leagueLevel, type LeagueLevel } from '../src/engine/metrics/leagueLevel.js';
...
function assertTarget(t: Target): void {
  ...
  for (const [i, v] of values.entries()) {
    expect(v, `seed ${SEEDS[i]} が許容帯外 ${describeValues(t)}`).toBeGreaterThanOrEqual(t.lo - t.tolerance);
    expect(v, `seed ${SEEDS[i]} が許容帯外 ${describeValues(t)}`).toBeLessThanOrEqual(t.hi + t.tolerance);
  }
}
