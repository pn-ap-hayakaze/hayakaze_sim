...
function playHalfInning(
  ...
  while (outs < 3) {
    ...
    events.push({
      ...
      basesBefore,
      outsAfter: outs,
      basesAfter: encodeBases(bases.first !== null, bases.second !== null, bases.third !== null),
      runs: runsScored,
      scoreDiffBefore,
    });
...
function clearBases(bases: Bases): Player[] {
  const runners = [bases.third, bases.second, bases.first].filter((p): p is Player => p !== null);
  bases.first = null;
  bases.second = null;
  bases.third = null;
  return runners;
...
function maybeChangePitcher(
  ...
  if (defense.pitcherPitches >= limit && armsLeft > 0) {
    ...
    if (highLeverage) {
      ...
      // 大差なら下位の中継ぎで消化する
      next =
        takeRandom(middle, Math.max(0, middle.length - 2), middle.length - 1, rng) ?? setup.pop();
    } else {
