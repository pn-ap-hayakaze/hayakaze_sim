...
printBatters(
  allBatters(season)
    .sort((a, b) => b.stats.sb - a.stats.sb)
    .slice(0, 5),
);

console.log('\n=== 防御率ランキング（規定投球回到達）===');
const qualifiedPitchers = allPitchers(season).filter((p) => p.stats.outs >= gamesPerSeason * 3);
...
console.log(
  '走者        0アウト  1アウト  2アウト   (現実の NPB: 無走者0アウト ≈ 0.45、満塁0アウト ≈ 2.2)',
);
...
console.log('\n=== 線形ウェイト（アウト基準・wOBA尺度）===');
console.log(
  '結果      得点価値   wOBAウェイト   (現実の MLB: 1B .88 / 2B 1.25 / 3B 1.58 / HR 2.03 / BB .69)',
);
...
console.log(
  `アウトの価値 ${ctx.outWeight.toFixed(3)} / wOBA尺度 ${ctx.wobaScale.toFixed(3)} / リーグwOBA ${fmtRate(ctx.leagueWoba)} / 1勝=${ctx.runsPerWin.toFixed(2)}点 / cFIP ${ctx.cFip.toFixed(2)}`,
);
...
if (leader) {
  ...
  const ratingMetrics: MetricSpec<Entry>[] = [
    {
      label: 'ミート',
      value: (e) => (e.player.ratings.batting.meetVsR + e.player.ratings.batting.meetVsL) / 2,
    },
    {
      label: 'パワー',
      value: (e) => (e.player.ratings.batting.powerVsR + e.player.ratings.batting.powerVsL) / 2,
    },
  ...
  const statMetrics: MetricSpec<Entry>[] = [
    ...
    {
      label: '本塁打率',
      value: (e) => e.stats.hr / e.stats.pa,
      format: (v) => `${(v * 100).toFixed(1)}%`,
    },
    {
      label: '四球率',
      value: (e) => e.stats.bb / e.stats.pa,
      format: (v) => `${(v * 100).toFixed(1)}%`,
    },
    {
      label: '三振率',
      value: (e) => e.stats.so / e.stats.pa,
  ...
  for (const c of percentileCard(leader, regulars, ratingMetrics)) {
    console.log(
      `${c.label.padEnd(8, '　')} ${c.display.padStart(6)}  ${percentileBar(c.percentile)} ${String(c.percentile).padStart(3)}`,
    );
  }
  console.log('--- 成績 ---');
...
check('防御率が 2.80〜3.90 に収まる', era(leaguePitching) > 2.8 && era(leaguePitching) < 3.9);
check(
...
check('3アウト目のプレーで得点が入っていない', ctx.runExpectancy.invariantViolations === 0);
check(
...
check(
  ...
  (() => {
    ...
    for (let bases = 0; bases < 8; bases++) {
      if (!(
        re[baseOutIndex(0, bases)] > re[baseOutIndex(1, bases)] &&
        re[baseOutIndex(1, bases)] > re[baseOutIndex(2, bases)]
      ))
        return false;
    }
...
const totalBatterWar = allBatters(season).reduce((a, e) => a + battingWar(e.stats, ctx), 0);
...
function row2(label: string, runs: number, weight: number): void {
  console.log(
    `${label.padEnd(5, '　')} ${runs.toFixed(3).padStart(8)} ${weight.toFixed(3).padStart(12)}`,
  );
}
...
function printBatters(entries: { player: Player; stats: BattingStats }[]): void {
  console.log(
    '選手              球団         打率  試合  打数  安打  本   点  盗  四球  三振   OPS',
  );
