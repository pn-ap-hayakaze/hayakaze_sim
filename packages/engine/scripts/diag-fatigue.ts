 ...
 */
import { advanceOneDay, createSeason, dailyRecovery, isSeasonOver } from '../src/league/season.js';
...
const sp = stat(spFatigueAtStart),
  rp = stat(rpFatigueAtEntry);
console.log(`=== 疲労モデル診断 ===`);
console.log(
  `先発の登板時疲労度: 平均${sp.m.toFixed(1)} 中央値${sp.p50.toFixed(0)} 90%点${sp.p90.toFixed(0)} 最大${sp.max.toFixed(0)}  (0なら完全回復で登板)`,
);
console.log(
  `救援の登板時疲労度: 平均${rp.m.toFixed(1)} 中央値${rp.p50.toFixed(0)} 90%点${rp.p90.toFixed(0)} 最大${rp.max.toFixed(0)}`,
);
console.log(
  `救援の登板 ${rpEntries}回のうち疲労30以上での登板 ${rpTiredEntries}回 (${((rpTiredEntries / rpEntries) * 100).toFixed(1)}%)`,
);
