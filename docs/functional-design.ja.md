...
オフシーズン（ポストシーズン後）: ドラフト（F9）、引退の発表、契約満了（シーズン1ではスタブ。全員が再契約）。
...
| scoutIds | シーズン1では球団ごとに少人数の固定スタッフ |
| policy | `POLICY` |
...
（重みベクトル。シーズン1では不活性）、`retirementTiming`（「引き際」）。
...
- **投げ抹消**: 先発後、推定スタミナまたは耐久が閾値未満なら AI が抹消し、10日後に再登録する。これが中10日以上の
  登板間隔 22〜23% を生む。
- **自由契約**（FR-3.4）: 支配下 → 自由。選手はフリープールに入る（シーズン1では契約なし。シーズン末に引退するが、
